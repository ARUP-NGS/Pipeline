...
public class TestMITOMAP extends TestCase {
	 ...
	 * 
	 * You need to comment out the ppl.setProperty() on ~line 35ish depending on which annotator you are testing
	 * Additionally, the src/test/java/annotation/testMITOMAP.xml file must be pointing to the correct annotator
	 * 
	 * author chrisk
	 */
	...
	public void testMITOMAP() {
		...
		try {
			...
			MITOMAP_rRNAtRNA annotator = (MITOMAP_rRNAtRNA)ppl.getObjectHandler().getObjectForLabel("GeneAnnotate");

			
			VariantPool vars = annotator.getVariants();	
			
//var:var6 are records from the MitoMap_coding.csv file
		/*	VariantRec var = vars.findRecord("MT",150,"C","T");
...
//var7: are from the MM_rRNAtRNA.csv file						
