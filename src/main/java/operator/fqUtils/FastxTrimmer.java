	...
	public void performOperation() throws OperationFailedException,
		...
		if (trim == null) {
			...
			throw new OperationFailedException(
					"Number of nucleotides to keep is required for FastxTrimmer operator",
					null);
		}
		...
		if (fastxTrim == null) {
			...
			throw new OperationFailedException(
					"No path to fastx_trimmer provided. Abort mission!", this);
		}
		String command_str;
		if (!inputFastq.substring(inputFastq.length() - 2, inputFastq.length())
				.equals("gz")) {
			command_str = fastxTrim + " -Q33 -l " + trim + " -i " + inputFastq
					+ " -o " + outputFastq;
